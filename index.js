...
function LGR(opts) {
    ...
    */
    this.addLevel('critical', 6000, {
        fg: 'red',
        'bg': 'yellow'
    }, 'CRITICAL!', process.stderr);
...
LGR.prototype.setErr = function (fileName) {
    if (fileName === undefined) {
        ...
        this.errorStream = FS.createWriteStream(fileName, {
            flags: 'a',
            encoding: null
        });
    }
...
LGR.prototype.flush = function () {
    ...
    if (_.get(this.outputStream, '_handle.fd', null) !== 1) {
        this.outputStream.end();
    }

    // Close the err stream if is not process.stdout
    if (_.get(this.errorStream, '_handle.fd', null) !== 2) {
